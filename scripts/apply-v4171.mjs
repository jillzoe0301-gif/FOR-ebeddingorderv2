...
const patchFile = new URL('./v4171.diff.gz.b64', import.meta.url);
...
if(original.includes('<title>FOR-e寢具訂購 V4.17.1</title>') && original.includes('function bindStableSearchInput(')){
  console.log('V4.17.1 search fix already applied.');
}else{
  const updated = applyUnifiedDiff(original, diff);
  const required = [
    '<title>FOR-e寢具訂購 V4.17.1</title>',
    'V4.17.1 Supabase 公開共用版',
    'function bindStableSearchInput(',
    "bindStableSearchInput('orderSearch','orders',renderOrders,root);",
    "bindStableSearchInput('recordsSearch','records',renderRecords,root);",
    "bindStableSearchInput('accountingSearch','accounting',renderAccounting,root);"
  ];
  required.forEach(marker=>{ if(!updated.includes(marker)) throw new Error(`V4.17.1 marker missing: ${marker}`); });
  fs.writeFileSync(target, updated, 'utf8');
  console.log('Applied FOR-e V4.17.1 stable search input fix.');
}
